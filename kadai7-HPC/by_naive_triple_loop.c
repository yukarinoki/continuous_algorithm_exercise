...
void run(int SIZE){
  ...
  for(int i=0; i<k; i++) for(int j=0; j<n; j++) IDX(c, k, i, j) = 0.0; 

  double alpha=1.0, zero=0.0; t1 = get_dtime();
  for(int i=0;i<m;i++) for(int j=0;j<m;j++) for(int l=0;l<m;l++) IDX(c, k, i, j) += IDX(a, k, i, l) * IDX(b, k, l, j);
